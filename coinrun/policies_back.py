...
def nature_cnn(scaled_images, **conv_kwargs):
    ...
    h = batch_norm(dropout_layer(h))
    
    h2 = activ(conv(h, 'c2', nf=64, rf=4, stride=2, init_scale=np.sqrt(2), **conv_kwargs))
    h2 = batch_norm(dropout_layer(h2))
...
def choose_cnn(images):
    ...
    if arch == 'nature':
        out, dropout_assign_ops = nature_cnn(scaled_images)
    elif arch == 'impala':
...
def random_choose_cnn(images, depths=[16,32,32]):
    def conv_rand(images,num_colors=3,kernel_size=3,randcnn_depth=3,rh=0.2):
        ...
        return out
    images = conv_rand(images)
    out,dropout_assign_ops = choose_cnn(images)
   
    return out,dropout_assign_ops

# new policy with randomize convoltion layer 
def random_impala_cnn_back(images,depths=[16,32.32]):
    """
    Model used in the paper "IMPALA: Scalable Distributed Deep-RL with 
    Importance Weighted Actor-Learner Architectures" https://arxiv.org/abs/1802.01561
    """
    ...
    #randcnn_depth = 3
    #kernel_size   = 3
    def conv_rand(images,num_colors=3,kernel_size=3,randomcnn_depth=3,rh=0.2):
        fan_in  = num_colors    * kernel_size * kernel_size
        fan_out = randcnn_depth * kernel_size * kernel_size
...
class RandomCnnPolicy(object):
    def __init__(self, sess, ob_space, ac_space, nbatch, nsteps, **conv_kwargs): #pylint: disable=W0613
        ...
        with tf.variable_scope("model", reuse=tf.AUTO_REUSE):  
            ...
            h, self.dropout_assign_ops = random_choose_cnn(scaled_images)  
            vf = fc(h, 'v', 1)[:,0]
            self.pd, self.pi = self.pdtype.pdfromlatent(h, init_scale=0.01)
            
        with tf.variable_scope("model", reuse=tf.AUTO_REUSE):
            #clean_h, _ = impala_cnn(scaled_images)
            clean_h, _ = random_choose_cnn(scaled_images)
