...
def learn(*, policy, env, nsteps, total_timesteps, ent_coef, lr,
            ...
            save_interval=10, load_path=None):
    """train ppo agent"""
    comm = MPI.COMM_WORLD
    rank = comm.Get_rank()
    ...
    for update in range(1, nupdates+1):
        ...
        if update % log_interval == 0 or update == 1:
            ...
            mpi_print('time_elapsed, run_t_total, train_t_total', tnow - tfirststart, run_t_total, train_t_total)
            mpi_print('episode_time/total_timesteps', update*nsteps, total_timesteps)

            mpi_print('10 episode length mean', ep_len_mean)
            mpi_print('10 episode mean reward', rew_mean_10)
            mpi_print('fps', fps)
